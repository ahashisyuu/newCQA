    ...
    def build_model(self):
        with tf.variable_scope('ai_cnn', initializer=tf.glorot_uniform_initializer()):
            ...
            with tf.variable_scope('interaction'):
                ...
                v = tf.get_variable('v', [200, 1], dtype=tf.float32)
                score = tf.keras.backend.dot(A, v)  # (B, L1, L2, 1)

                # convolution
                info = tf.layers.conv2d(score, 1, 5, activation=tf.nn.relu)  # (B, NL1, NL2, 1)
                eroll_info = tf.reshape(info, [self.N, -1])
                info_top_k, _ = tf.nn.top_k(eroll_info, k=20)  # (B, k)
                print(info_top_k)

            output = tf.layers.dense(info_top_k, self.args.categories_num, activation=tf.identity)
