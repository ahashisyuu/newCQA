...
class TriangularCell(LayerRNNCell):
    def __init__(self,
        ...
        self._state_size = TriangularStateTuple(self.sent1_length * self.dim,  # sentences
                                                ...
                                                self.dim,  # sentence hidden states
                                                self.dim,
                                                self.dim)
    ...
    def build(self, inputs_shape):
        ...
        if self.use_bias:
            ...
            self.r_gate_bias = self.add_variable(name="r_gate_bias",
                                                 shape=[1],
                                                 initializer=tf.constant_initializer(bias_init, dtype=tf.float32))
            self.att_r_bias = self.add_variable(name="att_r_bias",
                                                shape=[self.num_units],
                                                initializer=tf.constant_initializer(bias_init, dtype=tf.float32))
            self.s_bias = self.add_variable(name="s_bias",
                                            shape=[self.num_units],
                                            initializer=tf.constant_initializer(bias_init, dtype=tf.float32))
            self.att_s_bias = self.add_variable(name="att_s_bias",
                                                shape=[self.num_units],
                                                initializer=tf.constant_initializer(bias_init, dtype=tf.float32))
    ...
    def call(self, inputs, state=None):
        s1_tm, s2_tm, s3_tm, \
            ...
            r1_h_tm, r2_h_tm, r3_h_tm, \
            s1_h_tm, s2_h_tm, s3_h_tm = state
        # s: (B, Lx * dim), values: (B, keys_num * dim),
        # r_h: (B, keys_num * dim), s_h: (B, dim)
        ...
        s1, s1_h = self.update_sentence(s1_tm, s1_h_tm, self.sent1_length, self.sent1_mask,
                                        values1, s2_tm, s2_h_tm, self.sent2_length, self.sent2_mask,
                                        values3, s3_tm, s3_h_tm, self.sent3_length, self.sent3_mask)
        s2, s2_h = self.update_sentence(s2_tm, s2_h_tm, self.sent2_length, self.sent2_mask,
                                        values2, s3_tm, s3_h_tm, self.sent3_length, self.sent3_mask,
                                        values1, s1_tm, s1_h_tm, self.sent1_length, self.sent1_mask)
        s3, s3_h = self.update_sentence(s3_tm, s3_h_tm, self.sent3_length, self.sent3_mask,
                                        values3, s1_tm, s1_h_tm, self.sent1_length, self.sent1_mask,
                                        values2, s2_tm, s2_h_tm, self.sent2_length, self.sent2_mask)
    ...
    def update_sentence(self, s0_tm, s0_h_tm, s0_len, s0_mask,
                        values1, s1_tm, s1_h_tm, s1_len, s1_mask,
                        values2, s2_tm, s2_h_tm, s2_len, s2_mask):
        # s_tm: (B, Lx * dim), s_h_tm: (B, dim), s_len: scalar, s_mask: (B, Lx) or None
        # r_h: (B, keys_num * dim)

        s0_tm = tf.reshape(s0_tm, [-1, s0_len, self.dim])
        s1_tm = tf.reshape(s1_tm, [-1, s1_len, self.dim])
        s2_tm = tf.reshape(s2_tm, [-1, s2_len, self.dim])

        values1 = tf.reshape(values1, [-1, self.keys_num, self.dim])
        values2 = tf.reshape(values2, [-1, self.keys_num, self.dim])

        # <1> GET att
        s0_r1 = self.relation_att(s0_h_tm, values1)
        s1_r1 = self.relation_att(s1_h_tm, values1)

        s0_r2 = self.relation_att(s0_h_tm, values2)
        s2_r2 = self.relation_att(s2_h_tm, values2)

        # <2> Transform sentence
        s1_s0 = self.trans_sent(s1_tm, s1_mask, s1_r1, s1_len,
                                s0_tm, s0_mask, s0_r1, s0_len)  # (B, L0, dim)
        s2_s0 = self.trans_sent(s2_tm, s2_mask, s2_r2, s2_len,
                                s0_tm, s0_mask, s0_r2, s0_len)

        # <3> UPDATE
        s0 = tf.concat([s0_tm, s1_s0, s2_s0], axis=2)  # (B, L0, 3*dim)
    ...
    def trans_sent(self, s1, s1_mask, s1_r, s1_len,
                   s0, s0_mask, s0_r, s0_len):
        # transform the content of s1 into s0
        # s1: (B, L1, dim),  s0: (B, L0, dim)
        # s1_mask: (B, L1),  s0_mask: (B, L0)
        # s1_r: (B, dim),    s0_r: (B, dim)

        s1_expanded = tf.tile(tf.expand_dims(s1, axis=2), [1, 1, s0_len, 1])  # (B, L1, L0, dim)
        s0_expanded = tf.tile(tf.expand_dims(s0, axis=1), [1, s1_len, 1, 1])
        s0_r_expanded = tf.tile(tf.expand_dims(tf.expand_dims(s0_r, axis=1), axis=1), [1, s1_len, s0_len, 1])
        ...
        #
        #   2):
        #     score = self.activation(temp_array)  # (B, L1, L0, 1)
        #
        # #==========
        ...
        output = tf.reduce_sum(alpha * s1_expanded, axis=1)
