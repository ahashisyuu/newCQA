...
class BertCQAModel:
    ...
    def create_memory(self, num_units):
        with tf.variable_scope("memory"):
            self._keys_embedding = tf.get_variable("keys_embedding", [3, num_units],
                                                   dtype=tf.float32,
    ...
    def build_model(self):
        with tf.variable_scope("inferring_module"):
            ...
            sr_cell = GRUCell(num_units=num_units, activation=tf.nn.relu)
            ...
            # or self.output = output
            self.output = [a for a in last_state[3:6]]
            # self.output = output

    def get_output(self):
        # return self.output
        with tf.variable_scope("relation_output"):
            output = tf.concat(self.output, axis=1)
